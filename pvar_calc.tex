...
\begin{definition}[Piecewise monotone functions](\cite{Qian}, Def. 3.2)\label{def:PM}
  ...
  if there are points $a=x_1<\dots<x_k=b$ for some finite $k$ such 
  that $f$ is monotone on each interval $[x_{j-1},x_j],\;j=1,2,\dots,k$. 
  Here for $j=1,2,\dots,k-1$, $x_j$ may be point $x-$ or $x+$.
  ...
  continuous piecewise monotone(CPM). The set of such functions is denoted $CPM=CPM[a,b]$. 
\end{definition}   
...
Let assume we want to calculate $p$-variation 
for arbitrary piecewise monotone functions $f$. 
According to definition(see ???) this means
that ...
could be divided ...

We developed tha algorithme..



\subsection{Algorithm}

The procedure of $p$-variation calculus goes as follows.

\begin{enumerate}
  \item \emph{Removing monotonic points}. 
