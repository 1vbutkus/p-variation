...
(see def.~\ref{def:Seq2Fun} and~\ref{def:pvarseq}).
Nonetheless, this algorithm could be used to calculate
the $p$-variation for arbitrary piecewise monotone function (see below). 
...
of the sample is given in \ref{def:pvarseq}.
On the other hand, using Cor.~\ref{cor:PM2CPM}
and Prop.~\ref{prop:pvar-max}, the $p$-variation of the sequence
could be expressed as
$$
v_p(X) = \max\left\{ \sum_{i=1}^k |X_{j_i} - X_{j_{i-1}}|^p :
  0=j_0<j_1<\dots<j_k=n,\; k=1,\dots,n  \right\}.
$$
% gal nekelti sita prie matematines dalies ir gireztai paargumentuoti  
  
  
 ...
 
With the same procedure $p$-variation of 
any piecewise monotone function could be calculated as well.
...
Let denote the sequence of values of the function $f$ 
at the points $\{x_{i}\}_{i=0}^{n}$ by
...
meaningless points in most effective way. If the point is identified 
as meaningless, then it is removed form the sample immediate. 
...
\begin{enumerate}
  ...
  \item \emph{Sample splitting}. The sample could be splitted  in 
   intervals that could analysed independently. 
...
According to the proposition ??? the p-variation is achieved on 
...
This step can by performed very quickly, since it does not include
any interdependent checking and could be done with vectorised functions.

% This checking could be done in multiple ways ...

% It is done by function ... 


% patikrinti tikruma ir veiksmingumas

\begin{lstlisting}

abs(diff(x, 2)) == MovingSum(abs(diff(x, 1)))

\end{lstlisting}

\subsubsection{Test points in small intervals}

The proposition ??? is very important for speeding up the calculus of p-variation. It states that if the point was not significant in any small interval, then it will not be significant in the full sample as well. 
This property allows to drop significant amount of points using quite simple operations.

Lets consider any interval with $4$ points, 
namely, lets examine interval $[X_{j}, X_{j+3}]$ for any $j=0,...,n-3$.
If 
\begin{equation}
  \sum_{i=j+1}^{j+3} |X_{i}-X_{i-1}|^p < |X_j-X_{j+3}|^p,
\end{equation}
then points $X_{j+1}, X_{j+2}$ are identified as meaningless, 
thus they should be doped out. 

We start analyse all the the intervals with $4$ points since any 
shorter interval is not worth for checking. If there are
only $2$ points, 
then the situation is trivial and there is nothing to check. 
If interval was $3$, then checking is meaningless since
$ |X_i  - X_{i+1}| > |X_i - X_{i+2}| $. The last property hold,
because all the monotonic points are removed, therefore,
if interval $[X_i, X_{i+1}]$ is increasing,
then interval  $[X_{i+1}, X_{i+2}]$ is decreasing (and wise versa).  
As a result, all the points in interval with $3$ points are meaningful.

After applying this kind of checking of all intervals with $4$ points
some of the points could be identified as meaningless therefore they are
groped out from the sample. As a result the sample changes, therefore
we can apply the same procedure again and find the new meaningless points. We can repeat the iterations until no new meaningless points are 
identified.

Then, the can go further and apply same checking for
intervals with $6$ points (the intervals with $5$ points are omitet base on similar arguments mentioned above).
...
new_x
while old_x != new_x
Check all intervals with 4 points
drop all meaningless points
if ( old_x != new_x) next
else
  Check all intervals with 6 points
  drop all meaningless points
if ( old_x != new_x) next
else
  Check all intervals with 8 points
  drop all meaningless points
...  
...
The sample could be splitted in intervals 

If there are known some meaningful points, then the sample 
could be splitted 



\subsubsection{Pseudo monotonic}
